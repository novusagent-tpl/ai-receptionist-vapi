...
### Controllare la disponibilità (evitare “no suitable table” da resOS)

- **Come si controlla:** il tool **check_openings(day, time)** è quello che verifica la disponibilità. La receptionist deve chiamarlo **prima** di create_booking: con day e time ottieni `available` (true/false), `reason` (es. full, not_in_openings, cutoff) e `nearest_slots`. **People non è un input di check_openings** (la capacità è “quanti tavoli/prenotazioni” in quello slot); people serve solo per create_booking.
- **Regola nel prompt:** la receptionist deve sempre chiamare check_openings(day, time) prima di create_booking e procedere con create_booking **solo se available=true**. Se available=false (es. reason=full), deve proporre gli orari in `nearest_slots` e non chiamare create_booking per quell’orario.
- **Allineare la capacità alla sala:** in KB (es. `kb/modena01.json`) il campo **max_concurrent_bookings** deve essere uguale al **numero reale di tavoli** (o posti prenotabili) per quello slot. Esempio: se hai 5 tavoli alle 20:00, imposta `max_concurrent_bookings: 5`. Quando ci sono già 5 prenotazioni per le 20:00, check_openings restituirà `available: false`, `reason: "full"` e `nearest_slots` con orari alternativi; la receptionist non chiamerà create_booking e non arriverai all’errore resOS 422 “no suitable table found”.
- **Se resOS restituisce comunque 422:** il backend mappa quell’errore in **NO_TABLE_AVAILABLE** con messaggio tipo “Non ci sono più tavoli disponibili per quell’orario. Posso proporle un altro orario?”. La receptionist è istruita a comunicare questo al cliente e a proporre un altro orario (e a chiamare di nuovo check_openings con il nuovo time se il cliente accetta).
